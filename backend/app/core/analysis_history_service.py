...
def _load_user_history(user_id: str) -> List[Dict[str, Any]]:
    path = _get_user_history_path(user_id)
...
def _save_user_history(user_id: str, history: List[Dict[str, Any]]) -> bool:
    path = _get_user_history_path(user_id)
    try:
    ...
    def save_analysis(
        ...
        if USE_DATABASE:
            ...
            try:
                ...
                logger.info(f"Saved analysis {analysis_id} for user {user_id} to MySQL")
                return analysis_id
            except Exception as e:
                logger.error(f"Failed to save to MySQL, falling back to file: {e}")
                ...
                logger.info(f"Saved analysis {analysis_id} for user {user_id} to file storage")
                return analysis_id
        else:
            # Use file storage
            history = _load_user_history(user_id)
    ...
    def delete_analysis(self, user_id: str, analysis_id: str) -> bool:
        ...
        if USE_DATABASE:
            ...
            if len(history) < original_length:
                return _save_user_history(user_id, history)
            return False
