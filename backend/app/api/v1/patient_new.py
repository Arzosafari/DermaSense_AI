...
async def get_analysis_history(
    ...
    user_id = user["username"]
    try:
        history = database_service.get_screening_history(user_id, limit)
        return {
            "user_id": user_id,
            "history": history,
            "total": len(history)
...
async def delete_analysis(
    ...
    user_id = user["username"]
    success = database_service.delete_screening(user_id, analysis_id)
...
async def get_analysis_report(
    ...
    existing_report = database_service.get_pdf_report(analysis_id)
    if existing_report:
        return {
            "user_id": user_id,
